      ...
      <div className="space-y-6">
        ...
        {previewSub && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/70 backdrop-blur-md" onClick={() => setPreviewSub(null)} />
            <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh]">
              {/* Modal header */}
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-white/8 flex-shrink-0">
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">
                    {previewSub.partner?.name || "Partner"}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    {campaign?.title || "Campaign submission"}
                    {previewSub.platform && (
                      <span className="ml-2 capitalize text-emerald-600 dark:text-emerald-400">{previewSub.platform}</span>
                    )}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {previewSub.submittedUrl && (
                    <a
                      href={previewSub.submittedUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border border-slate-200 dark:border-white/15 bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 transition-all"
                    >
                      <FaEye className="h-3 w-3" /> Open
                    </a>
                  )}
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {previewSub.submittedUrl ? (
                  <SocialEmbed
                    url={previewSub.submittedUrl}
                    platform={previewSub.platform}
                    className="w-full"
                  />
                ) : (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400 dark:text-slate-500">
                    <FaEye className="h-8 w-8 mb-3 opacity-40" />
                    <p className="text-sm">No post URL submitted</p>
                  </div>
                )}
              </div>
              {/* Approve / Reject actions */}
              {["PENDING", "PENDING_REVIEW"].includes(previewSub.status) && (
                <div className="flex gap-3 px-5 py-4 border-t border-slate-200 dark:border-white/8 flex-shrink-0">
                  <button
                    onClick={() => { handleWorkAction(previewSub.id, "REJECTED"); setPreviewSub(null); }}
                  ...
                  >
                    {actionLoading === previewSub.id ? <FaSpinner className="animate-spin h-3.5 w-3.5" /> : <FaTimesCircle className="h-3.5 w-3.5" />}
                    Reject
                  </button>
                  <button
                    onClick={() => { handleWorkAction(previewSub.id, "APPROVED"); setPreviewSub(null); }}
                    disabled={actionLoading === previewSub.id}
                  ...
                  >
                    {actionLoading === previewSub.id ? <FaSpinner className="animate-spin h-3.5 w-3.5" /> : <FaCheckCircle className="h-3.5 w-3.5" />}
                    Approve Work
                  </button>
                </div>
              )}
